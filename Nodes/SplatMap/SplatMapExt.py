...
class SplatMapExt:
	"""
	SplatMapExt is an extension for the SplatMap node.
	"""
	...
	def OnStart(self):
		...
		# Gather menu names from the 'attributes' DAT table
		tempList = []
		datmenu = op('attributes')
		for i in range(datmenu.numRows - 1):
			tempList.append(datmenu[i+1, 0].val)

		# Update SourceMenuNames and menu if changed
	...
	def OnPulse(self, name):
		if name == 'Help':
			# open html browser
			mod.webbrowser.open('https://github.com/CraftKontrol/GroundGen-for-Touchdesigner/wiki/splatmap')
		pass
